...
class Matching(torch.nn.Module):
    ...
    def __init__(self, config={}):
        ...
        extractor = list(config)[0]
        if extractor == "superpoint":
            print("---------Using SuperPoint & SuperGlue ------------")
            self.Extractor = SuperPoint(config.get('superpoint', {}))
            self.Matcher = SuperGlue(config.get('superglue', {}))
        elif extractor == "sift":
            print("---------Using SIFT & NearestNeighbor ------------")
            self.Extractor = SIFT(config.get('sift', {}))
            self.Matcher = NearestNeighbor(config)
